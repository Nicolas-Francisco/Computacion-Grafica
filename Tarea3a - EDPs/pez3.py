...
def createFish(color1, color2):
    ...
    aleta1.transform = tr.matmul([tr.translate(0.1, 0.6, 0), tr.scale(0.4, 0.15, 0.05), tr.rotationZ(-np.pi/6)])
    aleta1.childs += [gpuCube2]
    
    aleta2 = sg.SceneGraphNode("wheel")
    aleta2.transform = tr.matmul([tr.translate(0.0, 0.6, 0), tr.scale(0.4, 0.15, 0.05), tr.rotationZ(np.pi/6)])
    aleta2.childs += [gpuCube2]
    
    aleta3 = sg.SceneGraphNode("wheel")
    aleta3.transform = tr.matmul([tr.translate(0.1, -0.6, 0), tr.scale(0.4, 0.15, 0.05), tr.rotationZ(np.pi/6)])
    aleta3.childs += [gpuCube2]
    
    aleta4 = sg.SceneGraphNode("wheel")
    aleta4.transform = tr.matmul([tr.translate(0.0, -0.6, 0), tr.scale(0.4, 0.15, 0.05), tr.rotationZ(-np.pi/6)])
    aleta4.childs += [gpuCube2]
    
    aleta5 = sg.SceneGraphNode("wheel")
    aleta5.transform = tr.matmul([tr.translate(-0.8, 0, 0.1), tr.scale(0.15, 0.05, 0.4), tr.rotationY(np.pi/6)])
    aleta5.childs += [gpuCube2]
    
    aleta6 = sg.SceneGraphNode("wheel")
    aleta6.transform = tr.matmul([tr.translate(-0.8, 0, -0.1), tr.scale(0.15, 0.05, 0.4), tr.rotationY(-np.pi/6)])
    ...
    Cube1.transform = tr.matmul([tr.translate(0, 0, 0), tr.scale(0.6, 0.6, 1)])
    Cube1.childs += [gpuCube1]
    
    Cube2 = sg.SceneGraphNode("chasis2")
    Cube2.transform = tr.matmul([tr.translate(0, 0, 0), tr.scale(1, 0.6, 0.6)])
    Cube2.childs += [gpuCube1]
    
    Cube3 = sg.SceneGraphNode("chasis2")
    Cube3.transform = tr.matmul([tr.translate(0, 0, 0), tr.scale(0.8, 0.8, 0.8)])
    Cube3.childs += [gpuCube2]
    
    Cube4 = sg.SceneGraphNode("chasis2")
    Cube4.transform = tr.matmul([tr.translate(0, 0, 0), tr.scale(1.2, 0.4, 0.4)])
    Cube4.childs += [gpuCube2]
    
    Cube7 = sg.SceneGraphNode("chasis2")
    Cube7.transform = tr.matmul([tr.translate(0, 0, 0), tr.scale(0.6, 1, 0.6)])
    Cube7.childs += [gpuCube1]
    
    Cube5 = sg.SceneGraphNode("chasis2")
    Cube5.transform = tr.matmul([tr.translate(0.5, 0.18, 0.225), tr.scale(0.1, 0.2, 0.1)])
    Cube5.childs += [gpuBlackCube]
    
    Cube6 = sg.SceneGraphNode("chasis2")
    Cube6.transform = tr.matmul([tr.translate(0.5, -0.18, 0.225), tr.scale(0.1, 0.2, 0.1)])
...
if __name__ == "__main__":
    ...
    redFishNode = createFish([0.52,0.25,0], [0.7,0.5,0.3])
