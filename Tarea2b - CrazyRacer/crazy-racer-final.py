...
if __name__ == "__main__":
    ...
    gpuCaja = es.toGPUShape(bs.createTextureCube("caja.jpg"),GL_REPEAT,GL_NEAREST)
    gpuLakitu = es.toGPUShape(bs.createTextureQuad("lakitu.png"),GL_REPEAT,GL_NEAREST)
    ...
    while not glfw.window_should_close(window):
        ...
        carZ = (max(posX[2],posY[2],posZ[2])+1)   # Asigno su altura nueva como el máximo de la posicion
        ...
        animTransform = tr.matmul([
            tr.translate(170,210, 50 + 20*math.sin(t1)),
            tr.uniformScale(40),
            tr.rotationZ(-t1)
        ])
        animTransform2 = tr.matmul([
            tr.translate(15, 15, 20 + 5*math.cos(t1)),
            tr.uniformScale(14),
            tr.rotationX(math.pi/2)
        ])
        ...
        glUniformMatrix4fv(glGetUniformLocation(textureShaderProgram.shaderProgram, "model"), 1, GL_TRUE, animTransform)
        textureShaderProgram.drawShape(gpuCaja)
        glUniformMatrix4fv(glGetUniformLocation(textureShaderProgram.shaderProgram, "model"), 1, GL_TRUE, animTransform2)
        textureShaderProgram.drawShape(gpuLakitu)
