...
def generateTextureNormalsCylinder(latitudes, image_filename, R = 1.0, z_top=1.0, z_bottom=0.0):
    ...
    # We generate a rectangle for every latitude, 
    for in range(latitudes):
        # d === c
        # |     |
        # |     |
        # a === b

        a = np.array([Rnp.cos(theta), Rnp.sin(theta), z_bottom])
        b = np.array([Rnp.cos(theta + dtheta), Rnp.sin(theta + dtheta), z_bottom])
        c = np.array([Rnp.cos(theta + dtheta), Rnp.sin(theta + dtheta), z_top])
        d = np.array([Rnp.cos(theta), Rnp.sin(theta), z_top])
